...
this.addToMySchedule = function(matchDay) {
	var mySchedule = this.getMySchedule();
	var existingMatchDay = this.findMatchDayInMySchedule(mySchedule, matchDay);
...
this.getMySchedule = function() { 
	var stored = localStorage["WCS:MySchedule"];
	if(stored === undefined) {
		return [];
	}
	var parsedWithStringDates = JSON.parse(stored);

	parsedWithStringDates.forEach(function(x){ x.date = new Date(x.date) });

	return parsedWithStringDates; 
};

this.save = function(mySchedule) {
	localStorage.setItem("WCS:MySchedule", JSON.stringify(mySchedule));
};
