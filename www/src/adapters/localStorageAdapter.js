...
;
	...
	localStorage.setItem("WCS:Schedule", JSON.stringify(schedule));
    };
...
this.getMatchDaysByTeam = function(code) {
	...
	this.getSchedule().matchDays.forEach(function(matchDay){
...
this.getSchedule = function(){
	var stored = localStorage["WCS:Schedule"];
	if(stored === undefined) {
		return [];
	}
	var parsedWithStringDates = JSON.parse(stored);

	parsedWithStringDates.matchDays.forEach(function(x){ x.date = new Date(x.date) });

	return parsedWithStringDates;
};

this.findMatchDayInMySchedule = function(mySchedule, matchDay){
...
this.removeFromMySchedule = function(matchId){
	var matchesInMySchedule = this.getMatchesInMySchedule();
	matchesInMySchedule = matchesInMySchedule.filter(function(m){ return m !== matchId});
	this.saveMatchesInMySchedule(matchesInMySchedule);
};


this.addToMySchedule = function(matchId){
	var matchesInMySchedule = this.getMatchesInMySchedule();
	matchesInMySchedule.push(matchId);
	this.saveMatchesInMySchedule(matchesInMySchedule);
};
...
this.getMySchedule = function() {
	var matchDaysInMySchedule = [];
	var that = this;
	this.getSchedule().matchDays.forEach(function(matchDay){
		var currentMatchDay = { "date" : matchDay.date };
		currentMatchDay.matches = [];
		matchDay.matches.forEach(function(match){
			if(that.isMatchInMySchedule(match.matchId)) {
				currentMatchDay.matches.push(match);
			}
		});
		if(currentMatchDay.matches.length > 0) {
			matchDaysInMySchedule.push(currentMatchDay);
		}
	});
	return matchDaysInMySchedule;
};

this.isMatchInMySchedule = function(matchId) {
	var matchesInMySchedule = this.getMatchesInMySchedule();
	return matchesInMySchedule.filter(function(id){ return id === matchId }).length > 0;
};
